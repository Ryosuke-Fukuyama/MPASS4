class ApplicationController < ActionController::Base
  ...
  def admin_required
    redirect_back(fallback_location: root_path) if @hospital.nil?
    redirect_back(fallback_location: health_interviews_path(@hospital)) unless (staff_signed_in? && current_staff.admin? && current_staff.hospital_id == @hospital.id) || master_signed_in?
  end
